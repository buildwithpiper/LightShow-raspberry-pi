...
{
  for (size_t i = 0; i < MATRIX_SIZE * 4; i += 4)
  {
    write(fd, (void *)(buf + i), 4);
  }
  write(fd, (void *)&clear_signal, 4); // otherwise the last row is brighter
  return;
}
...
int main(void)
{
  ...
  while (1)
  {
    // need to write RED | BLUE | GREEN | ROW , which is 4 bytes once per 8 rows
...
/*
TODO:
- figure out brightness controls?
- 3 bit -> 6 bit color?
- Text API?
- 
FORMAT SPECIFIER?
-> drawbuffer? (kinda done!)
-> fun spirals! 
