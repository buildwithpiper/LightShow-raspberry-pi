...
//change this to "pipmouse", "zombie", or "piperbot"
const uint8_t *imageData = pipmouse;
...
int main(int argc, char **argv)
{
  ...
  {
    char *opt = argv[1];
    if (!strcmp(opt, "pipmouse"))
    {
      imageData = pipmouse;
    }
    else if (!strcmp(opt, "piperbot"))
    {
      imageData = piperbot;
    }
    else if (!strcmp(opt, "zombie"))
    {
      imageData = zombie;
    }
    else
    {
      fprintf(stderr, "I don't know \"%s\". Try \"pipmouse\", \"zombie\", or \"piperbot\"?\n", opt);
      return EXIT_FAILURE;
    }
  }
